...
public sealed class ApiTests : IDisposable
{
    ...
    public async Task StreamsHeadAndRangesFromSparseLargeFile()
    {
        ...
        using var head = new HttpRequestMessage(HttpMethod.Head, $"/api/v1/files/{id}/content"); using var headResponse = await client.SendAsync(head, HttpCompletionOption.ResponseHeadersRead); Assert.Equal(HttpStatusCode.OK, headResponse.StatusCode); Assert.Equal(length, headResponse.Content.Headers.ContentLength); await using (var headBody = await headResponse.Content.ReadAsStreamAsync(TestContext.Current.CancellationToken)) { var probe = new byte[1]; Assert.Equal(0, await headBody.ReadAsync(probe, TestContext.Current.CancellationToken)); } Assert.Contains("bytes", headResponse.Headers.AcceptRanges); Assert.NotNull(headResponse.Headers.ETag); Assert.NotNull(headResponse.Content.Headers.LastModified); var etag = headResponse.Headers.ETag!;
